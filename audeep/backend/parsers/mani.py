...
class ManiParser(LoggingMixin, Parser):
    """
    Parser for the Mani data set.
    """
    ...
    def _metadata(self) -> pd.DataFrame:
        ...
        The Mani_metadata_anno.csv file is read only once and cached.
        
        Returns
        -------
        pandas.DataFrame
            The metadata contained in the Mani_metadata_anno.csv file as a pandas DataFrame
        ...
        if self._metadata_cache is None:
            self._metadata_cache = pd.read_csv(str(self._basedir / "Mani_metadata_anno.csv"),delimiter=";", header=0, usecols=['ManiaLevel', 'WaveFilename'])
    ...
    def _cv_setup(self) -> Sequence[pd.DataFrame]:
        ...
        if self._cv_setup_cache is None:
            ...
            for train, test in skf.split(a['WaveFilename'], a['ManiaLevel']):
    ...
    def can_parse(self) -> bool:
        """
        Checks whether the data set base directory contains the Mani data set.
        
        Currently, this method checks for the presence of a Mani_metadata_anno.csv file
        in the data set base directory, and the presence of Mani directories
        
        Returns
        -------
        bool
             True, if the data set base directory contains the Mani data set, False otherwise
        """
        anno_exists = (self._basedir / "Mani_metadata_anno.csv").exists()
        mani_dir_exists = (self._basedir / "Mani").exists()

        return anno_exists and mani_dir_exists
    ...
    def parse(self) -> Sequence[_InstanceMetadata]:
        ...
        for index, row in metadata.iterrows():
            ...
            label_nominal = str(row['ManiaLevel'])
